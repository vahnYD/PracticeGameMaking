...
@onready var HPBar: TextureProgressBar = $Control/HPBar
...
func _ready():
	...
	HPBar.value = abs(current_health/max_health) * 100
	DmgNumberStaticPos = DmgNumberPos.position
	#above are all value reset
...
func take_damage(amount: float):
	...
	if amount < 100:
		...
		dmgIntense = 2
	# decides if the attack is intense or not
...
func _damage_display_handle():
	...
	_this_tween.tween_property(DamageNumberLabel,"modulate", Color.TRANSPARENT, dmgIntense + 1.5)
